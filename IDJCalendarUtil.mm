...
+ (NSDateComponents *)toSolarDateWithYear:(NSUInteger)_year month:(NSString *)_month day:(NSUInteger)_day {
    ChineseDate chineseDate=ChineseDate((int)_year, (int)[IDJCalendarUtil monthFromMineToCppWithYear:_year month:_month], (int)_day);
...
+ (NSString *)monthFromCppToMineWithYear:(NSUInteger)_year month:(NSUInteger)_month {
    ...
    if (run==0) {
        ...
        if (_month==run+1) {
            return [NSString stringWithFormat:@"%@-%u", CHINESE_MONTH_LUNAR, _month-1];
        } else if(_month<=run) {
